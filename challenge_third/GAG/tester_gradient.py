...
"""
Created on Wed Dec 06 13:53:20 2017
...
filenames = ['C:/Users/helga/Dropbox/Andri/more_than_twenty/' + f
...
#    print
    ...
    #                gray = clahe.apply(gray)
                    ...
                    frames_lsh.append(str(imagehash.average_hash(Image.fromarray(gray),
                            hash_size=10)))
                elif counter > length / divider and counter < length \
                    - length / divider:
...
spectral = cluster.SpectralClustering(n_clusters=params['n_clusters'],
        eigen_solver='arpack', affinity='nearest_neighbors').fit(X)

dbscan = cluster.DBSCAN(eps=params['eps']).fit(X)

affinity_propagation = \
    cluster.AffinityPropagation(damping=params['damping'],
                                preference=params['preference']).fit(X)

average_linkage = cluster.AgglomerativeClustering(linkage='average',
        affinity='cityblock', n_clusters=params['n_clusters'],
